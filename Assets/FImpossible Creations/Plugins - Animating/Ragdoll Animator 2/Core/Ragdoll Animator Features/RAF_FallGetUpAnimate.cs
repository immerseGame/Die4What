...
namespace FIMSpace.FProceduralAnimation
{
    ...
    {
        private FUniversalVariable springPowerOnFallV;
        private FUniversalVariable durationV;
        private FUniversalVariable fallClipNameV;
        private FUniversalVariable fallTransitionV;
        private FUniversalVariable fallStateLayerV;
        private FUniversalVariable onFallEventV;
        private FUniversalVariable getUpFacedownClipNameV;
        private FUniversalVariable getUpFromBackClipNameV;
        private FUniversalVariable getUpCrossfadeV;
        private FUniversalVariable getUpAnimatorLayer;
        private FUniversalVariable getUpEventV;
        private FUniversalVariable ragdolledPropertyV;
        private FUniversalVariable repositionBaseTransformV;
        private FUniversalVariable findRigidbodyV;
        private FUniversalVariable bodyVelocityV;
        private FUniversalVariable supportGetupRestoreV;
        private FUniversalVariable getupRestoreClipStateV;

        //FUniversalVariable ragdollStandupBlendDuration;
        private FUniversalVariable modeV;

        private int _ragProperty = -1;
        private int _fallClipState = -1;
        private int _getupFaceState = -1;
        private int _getupBackState = -1;
        ...
        private ERagdollGetUpType getupType = ERagdollGetUpType.None;
        ...
        private void CallGetUpAnimation( RagdollHandler handler, int getupHash )
        {
        ...
        public override void Editor_InspectorGUI( SerializedProperty handlerProp, RagdollHandler ragdollHandler, RagdollAnimatorFeatureHelper helper )
        {
            ...
            var getUpAnimatorLayer = helper.RequestVariable( "GetUpLayer", 0 );
            ...
            if( ragdollHandler.Mecanim != null )
            {
                // Animation State Crossfade
                var fallClipNameV = helper.RequestVariable( "Fall Animation:", "Animator State Name" );
                ...
                if( string.IsNullOrWhiteSpace( fallClipNameV.GetString() ) == false )
                {
                    EditorGUILayout.BeginHorizontal();
            ...
            EditorGUILayout.BeginHorizontal();
            if( GUILayout.Button( new GUIContent( EditorGUIUtility.IconContent( "Animator Icon" ).image ), EditorStyles.label, GUILayout.Height( 18 ), GUILayout.Width( 20 ) ) ) { }
            var getUpFacedownClipNameV = helper.RequestVariable( "Get Up Face Down:", "Get Up Face Down" );
            ...
            var getUpFromBackClipNameV = helper.RequestVariable( "Get Up From Back:", "Get Up From Back" );
            ...
            if( ( !string.IsNullOrWhiteSpace( getUpFacedownClipNameV.GetString() ) || !string.IsNullOrWhiteSpace( getUpFromBackClipNameV.GetString() ) ) )
            {
                EditorGUILayout.BeginHorizontal();
            ...
            if( supportGetupRestoreV.GetBool() )
            {
                ...
                var getupRestoreClipStateV = helper.RequestVariable( "On Restore Animation:", "" );
